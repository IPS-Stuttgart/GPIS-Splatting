...
Use `run_paper_grade_3dgs_comparison` to consolidate trained-3DGS and GPIS-variant evidence across scenes into a paper-facing table.
...
- FPS and VRAM from a gsplat render manifest or explicit performance CSV
...
```bash
run_paper_grade_3dgs_comparison \
...
`paper_3dgs_paper_table.csv` is the compact paper-facing table with mean ± std columns for PSNR, SSIM, LPIPS, F-score, Chamfer, Gaussian count, FPS, and VRAM.
