...
The repository intentionally keeps the GPIS, splat conversion, confidence calibration, diagnostics, and reproducibility layers explicit. CUDA training/rendering for full 3DGS remains an external interop path rather than a vendored renderer.
...
src/gpis_splatting/
  ...
  confidence calibration, zero-level initialization, 3DGS interop, and reports.
