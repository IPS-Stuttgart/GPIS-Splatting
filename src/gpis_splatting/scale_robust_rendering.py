...
def run_scale_robust_3dgs_experiment(
    *,
    ...
    """Render trained 3DGS models across test scales and rasterizer modes."""
    if (manifest_path is None) == (input_ply_path is None):
        raise ValueError("Pass exactly one of manifest_path or input_ply_path.")
    scale_values = normalize_scale_values(scales)
    mode_values = normalize_rasterize_modes(rasterize_modes)
    manifest = load_or_build_manifest(manifest_path=manifest_path, input_ply_path=input_ply_path, method_name=method_name)
    ...
    for rasterize_mode in mode_values:
        for scale in scale_values:
            ...
            for row in manifest.itertuples(index=False):
                ...
                prediction_subdir = Path(rasterize_mode) / label / variant / split / f"ours_{iteration}" / "renders"
                result = render_scaled_3dgs_ply_with_gsplat(
                    input_ply_path=point_cloud_path,
                    scene_dir=scene_root,
                    output_dir=render_root / prediction_subdir,
                    scale=scale,
                ...
                render_rows.append(render_manifest_row(row, method_name, point_cloud_path, prediction_subdir, rasterize_mode, scale, label, result))
                metric_rows.extend(
    ...
    status = {
        ...
        "split": split,
        "scales": [float(value) for value in scale_values],
    ...
    report_path.write_text(format_scale_robust_report(status, summary), encoding="utf-8")
    return {
...
def render_scaled_3dgs_ply_with_gsplat(
    *,
    ...
    if rasterize_mode not in RASTERIZE_MODES:
        raise ValueError(f"Unsupported rasterize mode {rasterize_mode!r}.")
    scene_root = Path(scene_dir)
    ...
    label = scale_label_value or scale_label(scale)
...
def evaluate_scaled_render_report(
    *,
    scene_dir: str | Path,
...
def summarize_scale_robust_metrics(metrics: pd.DataFrame, *, render_manifest: pd.DataFrame) -> pd.DataFrame:
    if metrics.empty:
        raise ValueError("No scale-robust metrics were computed.")
    grouped = (
        metrics.groupby(["method", "variant", "variant_kind", "rasterize_mode", "scale", "scale_label"], dropna=False)
        .agg(image_count=("frame_index", "count"), mean_psnr=("psnr", "mean"), mean_ssim=("ssim", "mean"), mean_lpips_vgg=("lpips_vgg", mean_or_nan))
        .reset_index()
    )
    meta_cols = [
        "method",
    ...
    available_meta = [col for col in meta_cols if col in render_manifest.columns]
    summary = grouped.merge(render_manifest[available_meta].drop_duplicates(), on=["method", "variant", "variant_kind", "rasterize_mode", "scale", "scale_label"], how="left")
    baseline = summary[summary["rasterize_mode"] == "classic"][["variant", "scale", "mean_psnr", "mean_ssim", "mean_lpips_vgg"]].rename(
        columns={"mean_psnr": "classic_mean_psnr", "mean_ssim": "classic_mean_ssim", "mean_lpips_vgg": "classic_mean_lpips_vgg"}
    )
    summary = summary.merge(baseline, on=["variant", "scale"], how="left")
...
def scale_prepared_frame(frame: dict[str, Any], scale: float) -> dict[str, Any]:
    ...
    width = int(round(float(scaled.get("width") or intrinsics.get("width")) * scale))
    height = int(round(float(scaled.get("height") or intrinsics.get("height")) * scale))
    width = max(1, width)
    height = max(1, height)
    scaled["width"] = width
...
def resize_target_image(path: str | Path, *, width: int, height: int) -> np.ndarray:
    ...
    resized = Image.fromarray(arr, mode="RGB").resize((width, height), resample=resample)
    return np.asarray(resized.convert("RGB"), dtype=np.float64) / 255.0


def load_or_build_manifest(*, manifest_path: str | Path | None, input_ply_path: str | Path | None, method_name: str) -> pd.DataFrame:
        ...
        [
            {
                ...
                "model_dir": str(path.parent.parent.parent) if path.parent.name.startswith("iteration_") else str(path.parent),
                "point_cloud_path": str(path),
                "retained_count": int(ply.vertex_count),
...
def render_manifest_row(row: Any, method_name: str, point_cloud_path: Path, prediction_subdir: Path, rasterize_mode: str, scale: float, label: str, result: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "source_gaussian_count": int(report["source_gaussian_count"]),
        "rendered_gaussian_count": int(report["rendered_gaussian_count"]),
        "retained_count": optional_attr(row, "retained_count"),
        "retention_fraction": optional_attr(row, "retention_fraction"),
        "gate_threshold": optional_attr(row, "gate_threshold"),
        "opacity_scaled": optional_attr(row, "opacity_scaled"),
        "report_path": str(result["report_path"]),
    }


def normalize_scale_values(scales: Iterable[float]) -> tuple[float, ...]:
...
def normalize_rasterize_modes(modes: Iterable[str]) -> tuple[str, ...]:
    values = tuple(str(mode).strip() for mode in modes if str(mode).strip())
    if not values:
        raise ValueError("At least one rasterize mode is required.")
    unsupported = sorted(set(values) - set(RASTERIZE_MODES))
    if unsupported:
        raise ValueError(f"Unsupported rasterize modes: {unsupported}.")
    return values
...
def optional_attr(row: Any, name: str) -> Any:
...
def format_scale_robust_report(status: dict[str, Any], summary: pd.DataFrame) -> str:
    lines = [
        "# Scale-Robust 3DGS Rendering",
        "",
        f"- Method: `{status['method']}`",
        f"- Scene: `{status['scene_dir']}`",
        f"- Split: `{status['split']}`",
        f"- Scales: `{', '.join(str(v) for v in status['scales'])}`",
        f"- Rasterize modes: `{', '.join(status['rasterize_modes'])}`",
        f"- Variants: `{status['variant_count']}`",
        f"- Render cells: `{status['render_count']}`",
        f"- Summary CSV: `{status['summary_path']}`",
    ]
    if not summary.empty:
        lines.extend(["", "## Summary", "", format_summary_table(summary)])
    return "\n".join(lines) + "\n"


def format_summary_table(summary: pd.DataFrame) -> str:
    lines = [
        "| variant | rasterize | scale | gaussians | retention | psnr | ssim | lpips | delta_psnr_vs_classic | delta_ssim_vs_classic |",
        "| --- | --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |",
    ]
    for row in summary.itertuples(index=False):
        lines.append(
            f"| `{row.variant}` | `{row.rasterize_mode}` | {row.scale:.6g} | {format_optional_number(getattr(row, 'rendered_gaussian_count', None))} | "
            f"{format_optional_number(getattr(row, 'retention_fraction', None))} | {format_optional_number(row.mean_psnr)} | {format_optional_number(row.mean_ssim)} | "
            f"{format_optional_number(row.mean_lpips_vgg)} | {format_optional_number(row.delta_psnr_vs_classic)} | {format_optional_number(row.delta_ssim_vs_classic)} |"
        )
    return "\n".join(lines)
