...
def render_3dgs_ply_with_gsplat(*, input_ply_path: str | Path, scene_dir: str | Path, output_dir: str | Path, split: str = "test", projection_convention: str = "auto", device: str = "auto", dtype: str | torch.dtype = "float32", opacity_mode: str = "logit", color_mode: str = "auto", sh_degree: int | str | None = "auto", strict_3dgs_fidelity: bool = True, background_mode: str = "auto", background_color: tuple[float, float, float] = (0.0, 0.0, 0.0), near_plane: float = 1e-2, far_plane: float = 1.0e10, radius_clip: float = 0.0, eps2d: float = 0.3, tile_size: int = 16, packed: bool = True, render_mode: str = "RGB", rasterize_mode: str = "classic", channel_chunk: int = 32, max_frames: int | None = None, max_gaussians: int | None = None, rasterization_fn: Any | None = None) -> dict[str, Any]:
    ...
    gaussians, color_info = gaussian_ply_to_gsplat_tensors(load_3dgs_ply(input_ply_path), device=dev, dtype=dt, opacity_mode=opacity_mode, color_mode=color_mode, sh_degree=sh_degree, strict_3dgs_fidelity=strict_3dgs_fidelity, max_gaussians=max_gaussians)
    ...
    bg = (1.0, 1.0, 1.0) if background_mode == "white" else ((0.0, 0.0, 0.0) if background_mode == "black" else tuple(float(v) for v in background_color))
    ...
    for frame_index in indices:
        ...
        kwargs = {"means": gaussians.means, "quats": gaussians.quats, "scales": gaussians.scales, "opacities": gaussians.opacities, "colors": gaussians.colors, "viewmats": camera.viewmat.unsqueeze(0), "Ks": camera.K.unsqueeze(0), "width": camera.width, "height": camera.height, "packed": packed, "near_plane": near_plane, "far_plane": far_plane, "radius_clip": radius_clip, "eps2d": eps2d, "tile_size": tile_size, "render_mode": render_mode, "rasterize_mode": rasterize_mode, "channel_chunk": channel_chunk, "camera_model": "pinhole", "backgrounds": background}
...
def gaussian_ply_to_gsplat_tensors(ply: Any, *, device: str | torch.device, dtype: str | torch.dtype, opacity_mode: str, color_mode: str = "auto", sh_degree: int | str | None = "auto", strict_3dgs_fidelity: bool = True, max_gaussians: int | None = None) -> tuple[GsplatGaussianTensors, dict[str, Any]]:
    ...
    dt = resolve_torch_dtype(dtype)
    dev = resolve_device(device)
    return GsplatGaussianTensors(torch.as_tensor(means, dtype=dt, device=dev), torch.as_tensor(quats, dtype=dt, device=dev), torch.as_tensor(scales, dtype=dt, device=dev), torch.as_tensor(opacities, dtype=dt, device=dev), torch.as_tensor(colors, dtype=dt, device=dev), int(ply.vertex_count)), info
...
def resolve_colors(v: np.ndarray, *, color_mode: str, requested_sh_degree: int | str | None, strict: bool) -> tuple[np.ndarray, dict[str, Any]]:
    ...
    has_sh = {"f_dc_0", "f_dc_1", "f_dc_2"}.issubset(names) and bool(rest_names)
    ...
    if mode == "rgb" or not {"f_dc_0", "f_dc_1", "f_dc_2"}.issubset(names):
        if strict and mode == "sh":
            raise ValueError("Strict SH rendering requested, but f_dc_0..2 are missing.")
        return vertex_colors(v).astype(np.float32), {"requested_color_mode": color_mode, "effective_color_mode": "rgb", "requested_sh_degree": requested_sh_degree, "effective_sh_degree": None, "max_sh_degree": max_degree}
    ...
    if coeff_count > 1:
        rest = np.stack([v[name] for name in rest_names[: 3 * (coeff_count - 1)]], axis=1).astype(np.float32)
        colors[:, 1:, :] = np.transpose(rest.reshape(v.shape[0], 3, coeff_count - 1), (0, 2, 1))
