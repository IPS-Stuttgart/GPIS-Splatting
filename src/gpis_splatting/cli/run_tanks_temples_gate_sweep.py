...
from gpis_splatting.real_geometry_sweep import default_gate_sweep_method_name, default_gate_thresholds, run_tanks_temples_gate_sweep
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run a gate-threshold geometry sweep for Tanks and Temples splats.")
    ...
    parser.add_argument("--gate-thresholds", type=float, nargs="+", default=list(default_gate_thresholds()))
...
def main(argv: list[str] | None = None) -> None:
    ...
    method_name = args.method_name or default_gate_sweep_method_name(args.splats_path)
    result = run_tanks_temples_gate_sweep(
        scene_dir=scene_dir,
    ...
    for row in result["status"]["best_by_f_score"]:
        print(
            f"best_f_score@{row['geometry_threshold']:.6g}: "
            f"gate>={row['gate_threshold']:.6g} f={row['f_score']:.6g} retention={row['retention_fraction']:.6g}"
        )
