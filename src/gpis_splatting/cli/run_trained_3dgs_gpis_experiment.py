...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Build a paper-grade trained-3DGS comparison table across scenes with PSNR/SSIM/LPIPS, F-score, Chamfer, Gaussian count, FPS, and VRAM."
    )
    parser.add_argument("--scene-config", required=True, help="JSON configuration with scene entries and metric artifact paths.")
    parser.add_argument("--output-dir", default=None, help="Override output_dir from the JSON config.")
    parser.add_argument("--comparison-name", default=None, help="Override comparison_name from the JSON config.")
    parser.add_argument("--prepared-root", default=None, help="Override prepared_root from the JSON config.")
    parser.add_argument("--split", default=None, help="Override the evaluation split.")
    parser.add_argument("--method-name", default=None, help="Override the method name used when computing photometry.")
    parser.add_argument("--prediction-subdir", default=None, help="Override the relative prediction subdirectory under each variant.")
