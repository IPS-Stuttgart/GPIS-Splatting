...
def refine_gpis_with_splat_feedback(
    ...
    *,
    ...
    gate, then promotes high-gate splats to heteroscedastic zero-level pseudo observations and refits GPIS.
    """
    if iterations < 0:
    ...
    for iteration in range(1, iterations + 1):
        gate = gpis_gate_for_splats(splats, current_model, epsilon, batch_size=batch_size)
        eligible = (gate >= min_gate) & ~selected_mask
        optical_mass = torch.clamp(splats.tau.to(dtype=gate.dtype), min=0.0)
        score = gate * optical_mass
...
def save_feedback_trace(path: str | Path, trace: list[dict[str, float | int]]) -> None:
