...
def annotate_psnr_constrained_pareto(
    ...
    *,
    ...
    validate_selection_inputs(comparison=comparison, baseline_variant=baseline_variant, psnr_drop_tolerance=psnr_drop_tolerance, objective=objective)
    ...
    ranking_pool = output.loc[frontier_index] if frontier_index else output.loc[feasible_index]
    ranked = rank_selection_candidates(ranking_pool, objective=objective)
...
def resolve_baseline_psnr(comparison: pd.DataFrame, *, baseline_variant: str) -> float:
    variants = comparison["variant"].astype(str)
    baseline = comparison[variants == str(baseline_variant)]
    if baseline.empty and "variant_kind" in comparison.columns:
        kinds = comparison["variant_kind"].astype(str)
        baseline = comparison[kinds == str(baseline_variant)]
    if baseline.empty:
...
def rank_selection_candidates(candidates: pd.DataFrame, *, objective: str) -> pd.DataFrame:
    ...
    if "mean_ssim" not in output.columns:
        output["mean_ssim"] = np.nan
    if "mean_lpips_vgg" not in output.columns:
        output["mean_lpips_vgg"] = np.nan
    if objective == "min_retained":
        columns = ["retained_count", "retention_fraction", "mean_psnr", "mean_ssim", "mean_lpips_vgg", "variant"]
        ascending = [True, True, False, False, True, True]
    elif objective == "max_psnr":
        columns = ["mean_psnr", "retained_count", "retention_fraction", "mean_ssim", "mean_lpips_vgg", "variant"]
        ascending = [False, True, True, False, True, True]
    elif objective == "max_ssim":
        columns = ["mean_ssim", "mean_psnr", "retained_count", "retention_fraction", "mean_lpips_vgg", "variant"]
        ascending = [False, False, True, True, True, True]
    elif objective == "min_lpips":
        columns = ["mean_lpips_vgg", "mean_psnr", "retained_count", "retention_fraction", "mean_ssim", "variant"]
...
def format_psnr_constrained_pareto_report(status: dict[str, Any], annotated: pd.DataFrame) -> str:
    ...
    if status["selected_variant"] is not None:
        lines.extend(
            [
                f"- Selected retained count: `{format_number(status['selected_retained_count'])}`",
                f"- Selected retention fraction: `{format_number(status['selected_retention_fraction'])}`",
                f"- Selected PSNR: `{format_number(status['selected_mean_psnr'])}`",
                f"- Selected PSNR drop: `{format_number(status['selected_psnr_drop_from_baseline'])}` dB",
                f"- Selected SSIM: `{format_number(status['selected_mean_ssim'])}`",
                f"- Selected LPIPS VGG: `{format_number(status['selected_mean_lpips_vgg'])}`",
            ]
        )
...
def format_selection_table(annotated: pd.DataFrame) -> str:
    ...
    rows = annotated.copy()
    if "pareto_selection_rank" in rows.columns:
        rows = rows.sort_values(["pareto_selected", "pareto_frontier", "pareto_selection_rank", "variant"], ascending=[False, False, True, True], na_position="last")
