...
def evaluate_scene(scene: dict[str, Any], *, config: Paper3DGSComparisonConfig, scene_index: int) -> dict[str, Any]:
    ...
    photometry = load_or_compute_photometry(scene, config=config, scene_dir=scene_dir, manifest_path=manifest_path, output_dir=output_dir)
    ...
    for variant in variants:
        ...
        row = {
            ...
            "gaussian_count": first_number(scalar_from_row(manifest_row, "retained_count"), lookup_number(photometry, variant, "retained_count"), lookup_number(performance, variant, "rendered_gaussian_count")),
...
def load_or_compute_photometry(scene: dict[str, Any], *, config: Paper3DGSComparisonConfig, scene_dir: Path, manifest_path: Path, output_dir: Path) -> pd.DataFrame:
...
def load_or_compute_geometry(scene: dict[str, Any], *, config: Paper3DGSComparisonConfig, scene_name: str, scene_dir: Path, manifest: pd.DataFrame, output_dir: Path) -> pd.DataFrame:
    ...
    for manifest_row in manifest.itertuples(index=False):
        ...
        threshold_row = selected_threshold[selected_threshold["group"].astype(str) == "all"].iloc[0] if "group" in selected_threshold.columns and "all" in set(selected_threshold["group"].astype(str)) else selected_threshold.iloc[0]
        summary_row = summary_table[summary_table["group"].astype(str) == "all"].iloc[0] if "group" in summary_table.columns and "all" in set(summary_table["group"].astype(str)) else summary_table.iloc[0]
        rows.append({
            "variant": variant,
...
def lookup_value(table: pd.DataFrame, variant: str, column: str) -> Any:
    return scalar_from_row(select_variant_row(table, variant), column)
...
def aggregate_comparison(comparison: pd.DataFrame) -> pd.DataFrame:
    ...
    for (variant, variant_kind), group in comparison.groupby(["variant", "variant_kind"], dropna=False):
        ...
        for metric in PAPER_METRICS:
            series = pd.to_numeric(group[metric], errors="coerce").dropna() if metric in group else pd.Series(dtype=float)
...
def write_artifacts(output_dir: Path, config: Paper3DGSComparisonConfig, comparison: pd.DataFrame, aggregate: pd.DataFrame, paper_table: pd.DataFrame, checks: pd.DataFrame, scene_statuses: list[dict[str, Any]], passed: bool) -> dict[str, Path]:
    ...
    paths["report"].write_text(format_report(status, aggregate, paper_table, checks), encoding="utf-8")
    return paths


def format_report(status: dict[str, Any], aggregate: pd.DataFrame, paper_table: pd.DataFrame, checks: pd.DataFrame) -> str:
    lines = [
